                        ...
                        <form key={2} className="flex flex-col gap-y-2" noValidate autoComplete="off">
                            ...
                            <div className="w-full flex items-center justify-between gap-x-8">
                                {formData?.buyWithToman ? ((((((giftcardData?.weight || 0) * (productAmount || 1)) * (priceInfo[0]?.buyPrice || 0)) + (giftcardData?.preparationCost || 0)) + (formData?.isPostDelivery ? giftcardData?.shippingCost || 0 : 0)) <= (userInfo?.tomanBalance || 0)) ? <LoadingButton type="button" variant="outlined" size="medium" className="rounded-lg px-10" disableElevation
                                    ...
                                    onClick={() => {
                                        router.push(`/panel/deposit?type=offline&amount=${((((((giftcardData?.weight || 0) * (productAmount || 1)) * (priceInfo[0]?.buyPrice || 0)) + (giftcardData?.preparationCost || 0)) + (formData?.isPostDelivery ? giftcardData?.shippingCost || 0 : 0)) - (userInfo?.tomanBalance || 0))}`,
                                            `/panel/deposit?type=offline&amount=${((((((giftcardData?.weight || 0) * (productAmount || 1)) * (priceInfo[0]?.buyPrice || 0)) + (giftcardData?.preparationCost || 0)) + (formData?.isPostDelivery ? giftcardData?.shippingCost || 0 : 0)) - (userInfo?.tomanBalance || 0))}`, { locale });
                                    }}>
