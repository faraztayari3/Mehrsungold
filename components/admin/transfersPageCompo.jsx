...
/**
 * TransfersPageCompo component that displays the TransfersPageCompo Component of the website.
 * @returns The rendered TransfersPageCompo component.
 */
const TransfersPageCompo = (props) => {
    ...
    const getTransactions = (search) => {
        ...
        ApiCall('/tradeable/transfer', 'GET', locale, {}, `${search ? `search=${search}&` : ''}sortOrder=0&sortBy=createdAt&limit=${transactionsLimit}&skip=${(pageItem * transactionsLimit) - transactionsLimit}`, 'admin', router).then(async (result) => {
    ...
    return (
        <div className=" flex flex-col gap-y-8">
            <h1 className="text-large-2">انتقال ها</h1>
                                    ...
                                    {transactions.map((data, index) => (
                                        <TableRow
                                            ...
                                            <TableCell className="rtl:rounded-r-2xl ltr:rounded-l-2xl border-none px-8 py-4 text-sm dark:text-white" scope="row">
                                                <LinkRouter legacyBehavior href={`/admin/panel/usersinglepage?id=${data.senderUser?._id}`}>
                                            ...
                                            <TableCell className="border-none px-8 py-4 text-sm dark:text-white" scope="row">
                                                <LinkRouter legacyBehavior href={`/admin/panel/usersinglepage?id=${data.receiverUser?._id}`}>
                                                    <a target="_blank" className="no-underline text-blue-400 hover:underline">
                                            ...
                                            <TableCell className="text-end rtl:rounded-l-2xl ltr:rounded-r-2xl border-none px-8 py-4 text-sm dark:text-white">
                                                {data.status == 'Accepted' ? data.type == 'PayLaterBuy' ? <Chip label="پرداخت شده" variant="outlined" size="small" className="w-full badge badge-success" /> :
                                                    <Chip label="تائید شده" variant="outlined" size="small" className="w-full badge badge-success" /> : ''}
                                                {data.status == 'Pending' ? data.type == 'PayLaterBuy' ? <Chip label="در انتظار پرداخت" variant="outlined" size="small" className="w-full badge badge-primary" /> :
                                                    <Chip label="در انتظار تائید" variant="outlined" size="small" className="w-full badge badge-primary" /> : ''}
                                                {data.status == 'Rejected' ? data.type == 'PayLaterBuy' ? <Chip label="پرداخت نشده" variant="outlined" size="small" className="w-full badge badge-error" /> :
                                                    <Chip label="رد شده" variant="outlined" size="small" className="w-full badge badge-error" /> : ''}
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
...
export default TransfersPageCompo;